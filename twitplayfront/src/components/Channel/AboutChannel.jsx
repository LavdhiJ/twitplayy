...
const AboutChannel = ({ owner = false }) => {
  ...
  const aboutChannel = useSelector((state) => state.user.aboutData); // or .userData?.about

  const finalUsername = owner ? currentUsername : routeUsername;

  useEffect(() => {
    if (finalUsername) {
      dispatch(getAboutChannel(finalUsername));
      console.log("Calling getAboutChannel with:", finalUsername);
    }
  }, [finalUsername, dispatch]);
